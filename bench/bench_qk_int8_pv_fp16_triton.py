...
from sageattention.triton.tmp import forward2, create_lut_tensor
...
for seq_len in {1024, 2048, 4096, 8192, 16384, 32768}:
    ...
    q_scale = torch.randn(
        batch_size, num_heads, (seq_len // 128), 1, dtype=torch.float16, device="cuda"
    )
    k_scale = torch.randn(
        batch_size, num_heads, (seq_len // 64), 1, dtype=torch.float16, device="cuda"
    )
    ...
    for i in range(5):
        forward2(q, k, v, q_scale, k_scale, lut_tensor, output_dtype=torch.bfloat16)
    # torch.cuda.synchronize()
...
#     for i in range(5):
#         forward_causal(q, k, v, q_scale, k_scale, output_dtype=torch.bfloat16)
#     torch.cuda.synchronize()
